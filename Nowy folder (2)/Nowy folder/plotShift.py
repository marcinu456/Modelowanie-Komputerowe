...
print("%sD %s directions diffusion ="%(sys.argv[1], 2 * int(sys.argv[1]))   , sum( [x2[i]/i for i in range(1,len(x2))] ) / len(x2) , 1)
print("%sD %s directions diffusion ="%(sys.argv[1], 3**int(sys.argv[1]) - 1), sum( [ x2_diagonal[i]/i for i in range(1,len(x2))] ) / len(x2), (1+np.sqrt(2))/int(sys.argv[1]))
plt.legend()    
plt.show()
